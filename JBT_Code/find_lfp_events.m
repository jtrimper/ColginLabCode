function evInds = find_lfp_events(eegData, eegSf, pkThresh, edgeThresh, minDur)
% function evInds = find_lfp_events(eegData, eegSf, pkThresh, edgeThresh, minDur)
%
% PURPOSE: 
...
%
% INPUT: 
%      eegData = eeg time series (filtered in range of interest)
...
if ~isempty(highAmpEdges)
    evInds = unique(highAmpEdges, 'rows'); %get rid of 'events' with the same edges
else
    evInds = [];
end
