function ripInfo = find_ripples(lfpStruct, coords)
...
%            ripFiltData*^ = lfp filtered in ripple frequency range (~150 - 300 Hz)
%                             *this one is not automatic output for the read_in_lfp function.
%                              You've got to filter the EEG and attach it to the structure
%            Optional Fields
%               thetaFiltData^ = lfp filtered in theta frequency range (6-12 Hz)
%               deltaFiltData^ = lfp filtered in delta frequency range (2-4 Hz)
...
%
% OUTPUT:
%       ripInfo = structure containing time-stamps and indices relative to EEG structure for when each ripple occurred
...
plotCheck = 1; %set to 1 to plot detected events for visual inspection


%% IDENTIFY TIMES WHEN RIPPLE POWER PEAKS

ripPow = zscore(abs(hilbert(lfpStruct.ripFiltEeg)));
...
if thetaDelta == 1
    ...
    deltaAmpTs = abs(hilbert(lfpStruct.deltaFiltEeg));
    thetaAmpTs = abs(hilbert(lfpStruct.thetaFiltEeg));
...
if plotCheck == 1
    ...
    for i = 1:numPlots
        ...
        plot(startInd-offSet:endInd+offSet, rescale(lfpStruct.ripFiltEeg(startInd-offSet:endInd+offSet),0,1), 'g');
        plot(startInd-offSet:endInd+offSet, rescale(ripPow(startInd-offSet:endInd+offSet),.5,1), 'm');
        plot(startInd:endInd, rescale(lfpStruct.ripFiltEeg(startInd:endInd),0,1), 'k');
