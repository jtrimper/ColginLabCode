...
%
% Version 1.6       Added Moving and Not Moving Distinction
% 23. March 2015     added by Ernie Hwaun 
%
% Created by Raymond Skjerpeng, CBM, NTNU, 2007
% Bug report: raymond.skjerpeng@ntnu.no
function NlxHeadDir1_6(inputFile)
...
% Minimum duration for moving. Should be in sec
param.duration = 1;
%__________________________________________________________________________
...
outputFile = strcat('out_',inputFile(1:ind(end)),'xls');
...
fprintf(fid2,'%s\t','Cell ');
fprintf(fid2,'%s\t','Mean Angle M');
fprintf(fid2,'%s\t','Mean Angle NM ');
fprintf(fid2,'%s\t','Vector Length M ');
fprintf(fid2,'%s\t','Vector Length NM ');
fprintf(fid2,'%s\t','% Good Samples');
fprintf(fid2,'%s\t','Total spikes');
fprintf(fid2,'%s\t','Spikes kept');
fprintf(fid2,'%s\t','% Spikes kept');
fprintf(fid2,'%s\t','Watson U^2_M');
for ii = 1:param.numDirBins-1
    fprintf(fid2,'%s%u\t','Bin ',ii);
end
fprintf(fid2,'%s%u\t','Bin ',param.numDirBins);
fprintf(fid2,'%s\t','Watson U^2_NM');
for ii = 1:param.numDirBins-1
    fprintf(fid2,'%s%u\t','Bin ',ii);
end
...
while ~feof(fid)
    ...
    folder_name = ['HeadDirection_M_NM-',date,'-out'];
    polarDir = strcat(cd,'\',folder_name,'\');
    	if ~isdir(polarDir)
			mkdir(polarDir);
        end
...
%     end
    ...
    %Trial number for naming the image file
    nTrial=nTrial+1;
    ...
    [ExtractedX,ExtractedY]=interporPos(posData.ExtractedX,posData.ExtractedY,param.maxTrackingGap,25);
    ...
    backX(indb) = NaN;
    backY(indb) = NaN;
    indp= find(ExtractedX==0);
    ExtractedX(indp) = NaN;
    ExtractedY(indp) = NaN;
    
    [frontX,frontY] = posMeanFilter(frontX,frontY);
    [backX,backY] = posMeanFilter(backX,backY);
    [ExtractedX,ExtractedY] = posMeanFilter(ExtractedX,ExtractedY);
    ...
    speed=sqrt((diff(ExtractedX)).^2+(diff(ExtractedY).^2))/(1/30);
    speed=smooth(speed,15);
    speed=[0; speed];
    
    % Find counterclockwise and clockwise turn index
    [Mindex_d, NMindex_d]=DistinguishMoving(speed,param.duration);
    ...
    for ii = 1:numCells
        ...
        % NM and M selective direction
        directionNM=direction;
        directionNM(~NMindex_d)=NaN;
        directionM=direction;
        directionM(~Mindex_d)=NaN;
        
        % Calculate the head direction for each spike
        spkDirNM = directionNM(spk2posInd);
        spkDirNM=spkDirNM(~isnan(spkDirNM)); %Remove spikes that are not during NM
        spkDirM = directionM(spk2posInd);
        spkDirM=spkDirM(~isnan(spkDirM)); %Remove spikes that art not during M
        
        % Calculate the mean firing direction
        [meanAngleNM,vectorLengthNM] = meanDirection(spkDirNM);
        [meanAngleM,vectorLengthM] = meanDirection(spkDirM);
        
        % Total number of position samples in the file
        totalNumPosSamples = length(direction(NMindex_d | Mindex_d));
        
        wU2_NM = watsonU2(spkDirNM,direction(NMindex_d));
        wU2_M = watsonU2(spkDirM,direction(Mindex_d));
        
        % Calculate the rate of each directional bin
        [polarRateNM,numPosSamplesNM,numSpikesNM] = polarPlot(spkDirNM,direction(NMindex_d),posData.t(NMindex_d),param.numDirBins,param.smooth,param.smoothFactor);
        [polarRateM,numPosSamplesM,numSpikesM] = polarPlot(spkDirM,direction(Mindex_d),posData.t(Mindex_d),param.numDirBins,param.smooth,param.smoothFactor);
        
        %Total number of spikes
        numSpikes=numSpikesNM+numSpikesM;
        
         % Percentage of good samples
        amountPosSamples = 100*(numPosSamplesNM+numPosSamplesM)/totalNumPosSamples;
        ...
        % Make the polar plot
        figure(1);
        polar(polarAngle,polarRateNM,'-r');
        hold on; polar(polarAngle,polarRateM,'-k');hold off
        grid off
        ...
        figFile = sprintf('%s%s%s',polarDir,tNames{ii}(1:end-2),'_',num2str(nTrial),'_polarPlot');
        ...
        fprintf(fid2,'%3.1f\t',meanAngleNM);
        fprintf(fid2,'%3.1f\t',meanAngleM);
        fprintf(fid2,'%3.1f\t',vectorLengthNM);
        fprintf(fid2,'%3.1f\t',vectorLengthM);
        ...
        fprintf(fid2,'%2.3f\t',wU2_NM);
        for jj = 1:param.numDirBins-1
            fprintf(fid2,'%3.2f\t',polarRateNM(jj));
        end
        fprintf(fid2,'%3.2f\t',polarRateNM(param.numDirBins));
        fprintf(fid2,'%2.3f\t',wU2_M);
        for jj = 1:param.numDirBins-1
            fprintf(fid2,'%3.2f\t',polarRateM(jj));
        end
        fprintf(fid2,'%3.2f\n',polarRateM(param.numDirBins));
    end
end
...
% Distinguish moving(M) and not moving(NM) 
function [Mindex_d, NMindex_d]=DistinguishMoving(speed,duration)

%minimum continonus samples. Assume the sampling rate is 25 samples/sec. Duration should be in sec
sample=duration*25;

%moving is defined as >=20cm/sec
Mindex=speed>=20;
%not moving is defined as <20cm/sec
NMindex=speed<20;

Mindex_d=zeros(size(Mindex,1),1);
NMindex_d=zeros(size(NMindex,1),1);
for i=1:size(speed,1)
    
    if i+sample-1<=size(speed,1)
    
        if Mindex(i:i+sample-1)==ones(size(sample,1),1)
            Mindex_d(i:i+sample-1)=ones(size(sample,1),1);
        end

        if NMindex(i:i+sample-1)==ones(size(sample,1),1)
            NMindex_d(i:i+sample-1)=ones(size(sample,1),1);
        end
        
    else
    
        if Mindex(size(Mindex,1)-sample+1:sample)==ones(size(size(Mindex,1)-sample+1:sample,1),1)
            Mindex_d(size(Mindex,1)-sample+1:sample)=ones(size(size(Mindex,1)-sample+1:sample,1),1);
        end
        
        if NMindex(size(NMindex,1)-sample+1:sample)==ones(size(size(NMindex,1)-sample+1:sample,1),1)
            NMindex_d(size(NMindex,1)-sample+1:sample)=ones(size(size(NMindex,1)-sample+1:sample,1),1);
        end
...
Mindex_d=logical(Mindex_d);
NMindex_d=logical(NMindex_d);
...
[posData.t,posData.ExtractedX,posData.ExtractedY,posData.targets] = Nlx2MatVT(posFile,fieldSelect,getHeader,extractMode);
