...
function [meanAng, meanVecLen, watson, polarRate, rayleighP] = NlxHeadDir_output(inputFile, tListFile)
...
fgetl(fid); %remove the ttfile name
nDirs=0;
...
while ~feof(fid)
	...
	tmp = strsplit(str,dir_delim);
	strchk=lower(tmp{end}(1:5));
   ...
   % disp(sprintf('%s%2.2f%s','Interpolate missing position samples. Maximum gap is ',param.maxTrackingGap,' seconds'))
    ...
    for ii = 1:numCells
        ...
        [~,~,~,rayleighP(ii,kk),~] = circularStat(spkDir);
        % Calculate the rate of each directional bin
        [polarRate{ii}(kk,:),numPosSamples,numSpikes] = polarPlot(spkDir,direction,posData.t,param.numDirBins,param.smooth,param.smoothFactor);
		...
		meanAng(ii,kk) = meanAngle;
		meanVecLen(ii,kk) = vectorLength;
		watson(ii,kk) = wU2;
    end
end
...
S = LoadSpikes_ray(F,dirPath,NaFile);
...
function S = LoadSpikes_ray(tfilelist, path, NaFile)
...
if ~isa(tfilelist, 'cell')
   error('LoadSpikes_ray: tfilelist should be a cell array.');
end
...
S = cell(nFiles, 1);
for iF = 1:nFiles
    %DisplayProgress(iF, nFiles, 'Title', 'LoadSpikes_ray');
    tfn = tfilelist{iF};
    % Check if file exist
