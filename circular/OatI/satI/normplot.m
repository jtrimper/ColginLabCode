function normplot(x,typ,symb,y)
%
% CALL: normplot(x)
%       normplot(x,typ)
%       normplot(x,typ,symb)
%
%       normplot by itself produces an empty normal probability paper
...
%plot([0,4],[log(-log(1-0.9935)),log(-log(1-0.9935))])
...
%    end
   %
   hp=0.5; y0=exp(-exp(-0));    %y0=1-exp(-exp(0));
   tt(1)=text((ul+hp),7,' 7');
   tt(2)=text((ul+hp),6,' 6');
   tt(3)=text((ul+hp),5,' 5');
   tt(4)=text((ul+hp),4,' 4');
...
%   vp=0.9935; tt=[];
%   tt(1)=text(x0,-log(-log(vp)),'1'); 
%   tt(2)=text(2,-log(-log(vp)),'2'); 
%   tt(3)=text(3,-log(-log(vp)),'3'); 
%   tt(4)=text(4,-log(-log(vp)),'4'); 
%   tt(5)=text(0,-log(-log(vp)),'0'); 
%   set(tt,'FontSize',6); set(tt,'VerticalAlignment','middle');
%   r=0.6; a=r; b=r*11.9/8.15;a0=0.2*a; b0=0.2*b; 
    set(gca,'Xtick',[1:20]);
    XTickLabels=[];
%    xtl=[[1:0.5:10],[14:2:50],[60:10:ul]]';
