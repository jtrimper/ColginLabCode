f6p3p4=figure('Name','Stephen''s test for equal direction','NumberTitle','off',...
'Position',[2 2 490 250],'Resize','off');
set(f6p3p4,'MenuBar','none');
axis('off');
ht=[];
ht(1)=text(0.02,0.9,'x vector: ');
ht(2)=text(0.02,0.7,'y vector: ');
%ht(2)=text(0.12,0.7,' % confidence interval: ');
set(ht,'FontName','Serif');
set(ht,'FontSize',15);
%hfr=uicontrol('Style','frame','String','What','Position',[35 15 170 170]);
hedittest1=uicontrol('Style','edit','Position',[170 195 175 20]);
hedittest2=uicontrol('Style','edit','Position',[170 157 175 20]);
set(hedittest1,'BackgroundColor',[1 1 1]);
set(hedittest2,'BackgroundColor',[1 1 1]);
%hchb1=uicontrol('Style','checkbox','String','Fisher and Lewis','Value','0','Position',[70 125 150 22]);
%hpb1=uicontrol('Style','pushbutton','String','Help','Callback','sg6p3p3b','Position',[400 195 40 27]);
%hchb2=uicontrol('Style','checkbox','String','Upton','Value','1','Position',[70 95 150 22]);
%set(hchb1,'BackgroundColor',[1 1 1]);
%set(hchb2,'BackgroundColor',[1 1 1]);
hpb=uicontrol('Style','pushbutton','String','OK','Callback','sg6p3p4a','Position',[340 15 40 27]);
