function [sigData, origData] = factRmAnova_line_perm_test_for_ud_intxn_mets(metric,fpass_opts,allF, desNumRandos, combosOfInt, valFlag)
% function [sigData, origData] = factRmAnova_line_perm_test_for_ud_intxn_mets(metric,fpass_opts,allF, desNumRandos, combosOfInt, valFlag)
%
% Function tests for an interaction and main effects for subregions and conditions, both within-subject measures, using a two-way factorial RM anova. 
%
% Shuffles are ACROSS rats (i.e., shuffling averages, not shuffling sweeps)
%
...
%
% A cluster based stats approach, using a two-way factorial repeated measures ANOVA for each point on the line
%
...
statThreshAlpha = .025; %statistical threshold (because there are two fRanges, split .05 in two)
initClustAlpha = 0.1; %alpha level for initial cluster detection
numMets = length(metric); %in case you're just doing one metric (e.g., wpli)
if numMets > 3
    numMets = 3; 
end
...
origPVals = zeros(size(metric(1).(valType),1), numMets, 3); %pre-allocate array for p values from the original data
for m = 1:numMets
    ...
    for v = 1:size(tmpVals,1);
        ipData = permute(tmpVals(v,:,:,:), [2 4 3 1]); %permute the data so it's in the right format for 'fact_rm_anova' (i.e., rats x condns x combos)
        [me, int] = fact_rm_anova(ipData);
        origPVals(v,m,1) = me.A.p; %main effect of subreg combo
        origPVals(v,m,2) = me.B.p; %main effect of condn
        origPVals(v,m,3) = int.p; %interaction 
    end
end
origPVals = 1 - origPVals; %flip p values so higher is better
...
numRandosInt = numRandosSubreg * numRandosCondn; %this many different ways to randomly assign the interaction data are possible
...
clustMaxs = zeros(2,numMets,3,numRandosInt); %pre-allocate for within group cluster maxes -- fRange x metric x effect x shuffles
...
for f = 1:2
    ...
    for m = 1:numMets %for each metric
...
if plotData == 1
    ...
    efNames = {'Main Effect: Conditions', 'Main Effect: Subreg Combo', 'Interaction'};
    if numMets == 3;
    ...
    efNames = {'Main Effect: Subreg Combo', 'Main Effect: Condition', 'Interaction'};
%     comboNames = {'DG/CA3', 'CA3/CA1', 'CA1/SUB'};
    ...
    for m = 1:numMets
        ...
        %--for main effect A (subreg combos)
...
%                 efLineY = repmat(.5, size(allF));
        ...
        %plot differences across subregion pairs (averaging across conditions)
        subplot(2,3,4) %
        clear tmpH
        for p = 1:3
        ...
        %plot differences across conditions (averaging across subregion pairs)
        subplot(2,3,5)
        clear tmpH
        for c = 1:4
