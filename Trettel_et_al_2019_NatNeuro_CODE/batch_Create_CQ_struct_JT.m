function output_struct = batch_Create_CQ_struct_JT(fc,recalc,fd_TT)
% 
%
...
S = LoadSpikes_JT(fc,'tsflag','ts'); % load the spikes for each .t file
...
% 			end
		...
		if isempty(fc_TT); % if we still do not have a tetrode file, we cant' create the CQ file
			...
			if badTT % if we got the wrong data loaded
				...
				output_struct(count_out).name = ['TT' Name.Tetrode '-' Name.Cluster];
				output_struct(count_out).avgWaveform = mWV;
				output_struct(count_out).stdWaveform = sWV;
				...
				CluSep.SNR = SNR;
				output_struct(count_out).ClusterSeperationStats = CluSep;
