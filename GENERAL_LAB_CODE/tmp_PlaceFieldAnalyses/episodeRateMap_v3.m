...
%
%
%  ADDED BY JB Trimper EXPLAIN ALL OF THE ADDITINAL OUTPUT VARIABLES
%          sparsity =
%       selectivity =
%    nFields_matrix =
%           avgRate =
...
function [information, sparsity, selectivity, nFields_matrix, avgRate, peakRate, fieldSize, zCoherence, peakRate_map, bursting, maps, avgSpeed] = episodeRateMap_v3(cellStack, startTime, stopTime, Fs, videoFile, id)
...
positionScaleFactor = 0.35; %JBT, for FMR1 phase precession preliminary
...
%     figure('name', ['Unit ' num2str(ii)], 'Position', [ 564 476 1115 420]);
    ...
    maps{ii,2} = xAxis;
    maps{ii,3} = yAxis;
    [nFields,fieldProp, fieldBins] = placefield(map,p,xAxis,yAxis,peak_all);
...
%     title('Path')
%     hold on;
%     for f = 1:size(fieldBins,1)
%         plot(yAxis(fieldBins{f,2}),-1*(xAxis(fieldBins{f,1})), '*');
%     end
...
function [nFields,fieldProp, fieldBins] = placefield(map,p,colAxis,rowAxis,peak_all)
