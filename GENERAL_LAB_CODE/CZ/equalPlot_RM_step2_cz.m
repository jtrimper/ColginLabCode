...
function equalPlot_RM_step2_cz(scale_max,rateMaps,timeMaps,peakRate,mapAxis,sessions,F,pathCoord,timeStamps,sLength,bins)
...
for ii = 1:numsession
    ...
    for jj=1:numCells
        ...
        if strcmp(img_text,'on')
            title(strcat(sprintf('%s','Path'),'?,cellnum'),'FontSize',20);
        end
        hold on;
        ...
        figImage = sprintf('%s%s%s%s',sessions{ii},strcat('placeFieldImages',num2str(scale_max),'\Path_'),F{jj}(1:end-2),'.fig');
        f = getframe(gcf);
        [pic, cmap] = frame2im(f);
        imwrite(pic,bmpImage,'bmp');
        saveas(2,epsImage,'epsc');
        pause(0.1)
    end
end
