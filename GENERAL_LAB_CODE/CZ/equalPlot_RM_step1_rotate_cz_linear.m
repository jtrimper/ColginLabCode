...
function [rateMaps,timeMaps,peakRate,mapAxis,sessions,F,pathCoord,timeStamps,spk_phase,phase] ...
    = equalPlot_RM_step1_rotate_cz(inFile,session_period,rorate_para,scale,sLength,bins)
...
%     index the running speed
    ...
    y_run = y(ind_run);
    t_run = t(ind_run);
    phase_run = phase(ind_run);
    ind_rest = find(vel0<3); %rest is slower than 3cm/s
    
    % Calculate the time map for this session
     timeMaps{ii,1} = findTimeMap(phase_run,t_run,bins,sLength,binWidth);
...
%     pathCoord{ii,3} = t;
    ...
    for jj=1:numCells
        ...
        if ~isa(S{jj},'ts') % Empty cell in this session
            ...
            [spk_phase{ii,jj}] = spikePos_v2(ts,phase,t);
            % Calculate rate map
            map = ratemap_phase(spk_phase{ii,jj},phase,t,h,mapAxis,vfs);
        end
         rateMaps{ii,jj} = map;
         peakRate(ii,jj) = max(max(map));
         timeStamps{ii,jj} = ts;
         spk_phase{ii,jj} = spk_phase;
         phase=phase;
...
function timeMap = findTimeMap(phase,post,bins,sLength,binWidth)
...
function map = ratemap_phase(spk_phase,phase,post,h,mapAxis,vfs)
        invh = 1/h;
        map=zeros(length(mapAxis));
        idx=0;
        for i = mapAxis
            idx=idx+1;
            map(idx)=rate_estimator_cz(spk_phase,i,invh,phase,post,vfs);
        end
end


% Calculate the rate for one position value
function r = rate_estimator_cz(spk_phase,i,invh,phase,post,vfs)
...
function [spk_phase,Ind] = spikePos_v2(ts,phase,post)
N = length(ts);
spk_phase = zeros(N,1);
Ind = zeros(N,1);
for ii = 1:N
