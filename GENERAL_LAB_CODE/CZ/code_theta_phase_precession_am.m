...
path = strcat('C:\DATA\Mouse 30\2014-12-10_10-52-25\begin3\');
file = strcat(path,'vt1.nvt');

subdir='C:\DATA\thetaphaseprecession';
savefile=strcat(subdir,'\Mouse30_141210_B3');

ttfile = 'CA1.txt';
save_path = strcat(path,'thetaphaseprecession','\');
...
img_text='on';
    ...
    if isdir(save_path)  % == if a folder, dirInfo(kk).isdir=1 ==
            if strcmp(save_path,strcat('thetaphaseprecession'))
                found = 1;
            end
    end
    if found==0
        mkdir(path,strcat('\','thetaphaseprecession','\'));
    end
...
for nc = 1:numcells
    ...
    [nFields,fieldProp] = placefield_CZ(temprate,p,xbins,peak_all);
