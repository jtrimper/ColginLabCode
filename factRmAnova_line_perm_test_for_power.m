function [sigData, origData] = factRmAnova_line_perm_test_for_power(metric,fpass_opts,allF, desNumRandos, valFlag)
% function [sigData, origData] = factRmAnova_line_perm_test_for_power(metric,fpass_opts,allF, desNumRandos, valFlag)
%
% Function tests for an interaction and main effects for subregions and conditions using a two-way factorial RM anova.
%
...
%   see function 'factRmAnova_line_perm_test_for_ud_intxn_mets' for interaction metrics (e.g., coherence, cross spec)
%
% A cluster based stats approach, using a factorial two-way RM ANOVA for each point on the line
%
...
for v = 1:size(origData,1);
    ...
    origPVals(v,1) = me.A.p; %main effect of subreg (within grps factor 1)
    origPVals(v,2) = me.B.p; %main effect of condn (within grps factor 2)
